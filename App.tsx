...
const App: React.FC = () => {
  ...
  // Carregar logs salvos do usuário ao iniciar ou trocar de usuário
  ...
  const handleLogin = (user: AuthUser) => {
    setCurrentUser(user);
    if (user.profile) {
      setChatSession(new NutritionChatSession(user.profile));
    }
  };
  ...
  const confirmPendingMacros = () => {
    ...
    setMessages(prev => [...prev, { role: 'model', text: `✅ **Análise de ${day} salva!** Os macros foram atualizados no seu Dashboard.` }]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 text-slate-800 font-sans relative pb-24">
          ...
          <div className="bg-white rounded-t-[40px] sm:rounded-3xl shadow-2xl max-w-md w-full p-8 border border-green-100 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Macros detectados para {pendingMacros.day}.</p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
              ...
              <button onClick={confirmPendingMacros} className="flex-1 py-4 bg-green-600 text-white font-bold rounded-2xl hover:bg-green-700 shadow-xl shadow-green-200 transition active:scale-95">Salvar no Diário</button>
            </div>
          </div>
        </div>
      )}
      ...
      {!isChatOpen && (
        <button onClick={() => setIsChatOpen(true)} className="fixed bottom-6 right-6 bg-green-600 text-white p-4 rounded-full shadow-2xl hover:bg-green-700 transition transform hover:scale-110 active:scale-95 z-40 flex items-center space-x-2 border-4 border-white">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           <div className="lg:col-span-8 space-y-8">
              ...
              <div className="bg-slate-900 p-8 rounded-[40px] shadow-2xl text-white relative overflow-hidden flex flex-col md:flex-row items-center justify-between">
                <div className="relative z-10 max-w-md text-center md:text-left">
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed mb-6">"O diário é sua ferramenta de autoconhecimento. Edite suas refeições sempre que lembrar de um detalhe novo!"</p>
           ...
           <div className="lg:col-span-4 space-y-6">
              ...
              <SuggestionBox onSend={(msg) => {
                const saved = JSON.parse(localStorage.getItem('nutri_suggestions') || '[]');
                localStorage.setItem('nutri_suggestions', JSON.stringify([...saved, { id: Date.now().toString(), userId: currentUser.id, userName: currentUser.name, message: msg, date: new Date().toLocaleString('pt-BR') }]));
              }} />
           </div>
        </div>
      </div>
